...
- [ ] PR 31, official-tap enforcement.
  Scope: reject `post_install` in `homebrew/core` and legacy flight blocks in
  `homebrew/cask`, remove their authoring documentation and mark the runtime
  entry points with commented `# odeprecated` markers. Keep this PR at the top
  of the stack and expect tap syntax to fail until PR 29 reaches zero hooks. Do
  not introduce runtime conflicts or warnings here; third-party compatibility
  remains intact.
