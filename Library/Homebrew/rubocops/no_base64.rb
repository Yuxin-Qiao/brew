    ...
    module Homebrew
      ...
      class NoBase64 < Base
        ...
        def on_send(node)
          ...
          elsif base64_const?(node.receiver)
            add_offense(node) do |corrector|
        ...
        def on_const(node)
          return unless base64_const?(node)
        ...
        def require_base64?(node)
          return false if node.receiver || !node.method?(:require)
        ...
        def base64_const?(node)
          return false unless node.is_a?(RuboCop::AST::ConstNode)
          return false if node.short_name != :Base64
